defmodule Expensive.CategoryRegexControllerTest do
  ...
  test "lists all entries on index", %{conn: conn} do
    ...
    assert html_response(conn, 200) =~ "Listing category_regexes"
  end

  test "renders form for new resources", %{conn: conn} do
    conn = get conn, category_regex_path(conn, :new)
    assert html_response(conn, 200) =~ "New category_regex"
  end
  ...
  test "shows chosen resource", %{conn: conn} do
    ...
    assert html_response(conn, 200) =~ "Show category_regex"
  end
  ...
  test "renders form for editing chosen resource", %{conn: conn} do
    ...
    assert html_response(conn, 200) =~ "Edit category_regex"
  end
